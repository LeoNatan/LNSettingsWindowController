...
CURRENT_VERSION=$(/usr/libexec/PlistBuddy LNSettingsWindowControllerExample/LNSettingsWindowControllerExample/Info.plist -c "Print CFBundleShortVersionString")
...
echo -e "\033[1;34mUpdating library version\033[0m"

/usr/libexec/PlistBuddy LNSettingsWindowControllerExample/LNSettingsWindowControllerExample/Info.plist -c "Set CFBundleShortVersionString $NEXT_VERSION" -c "Set CFBundleVersion 1"
...
gh release create --repo LeoNatan/LNSettingsWindowController "$NEXT_VERSION" --title "$NEXT_VERSION" --notes-file "${RELEASE_NOTES_FILE}"
